...
dotenv.config();
const lesgoRegister = (firstName, lastName, email, password, refferal, idPuzzle, answerquest, proxy) => new Promise ((resolve, reject) => {
...
(async() => {
    ...
    const checkReff = await checkRefferal(refferal, proxy);
    ...
    try{
      ...
      let status = false;
      while(status == false){
        const getIddPuzzle = await getIdPuzzle(proxy);
        const idPuzzle = getIddPuzzle.puzzle_id;
        //console.log(idPuzzle)
        const get64Puzzle = await getBase64Puzzle(idPuzzle, proxy);
        ...
        const register = await lesgoRegister(firstname, lastname, email, password, refferal, idPuzzle, answerques, proxy);
    ...
    //  console.log(key)
      ...
      do{
      try{
          login = await loginAcc(email, password, dateStringISO, idPuzzleLogin, answerquest, proxy);
      }catch(error){
      ...
      do{
      try{
        getTwitterUpdate = await twitterUpdate(login.data.token, proxy);
      }catch(error){
      ...
      do{
      try{
        getdiscordUpdate = await discordUpdate(login.data.token, proxy);
      }catch(error){
      ...
      do{
      try{
        gettelegramUpdate = await telegramUpdate(login.data.token, proxy);
      }catch(error){
